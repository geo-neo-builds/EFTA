        ...
        <form onSubmit={runSearch} className="flex flex-col gap-3 mb-6">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded border border-zinc-300 px-4 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-zinc-400"
            />
            <button
              type="submit"
              disabled={loading || !query.trim()}
              className="rounded bg-zinc-900 px-4 py-2 text-white text-sm disabled:opacity-50"
            >
              {loading ? "…" : "Search"}
            </button>
          </div>
        ...
        <ul className="space-y-3">
          {results.map((hit) => (
            <li
              ...
              className="rounded border border-zinc-200 bg-white p-4 hover:border-zinc-400 transition"
            >
              <div className="flex items-baseline justify-between gap-4 mb-1">
                <Link
                  href={`/doc/${hit.doc_id}?page=${hit.page_number}`}
                  className="font-mono text-sm text-zinc-900 hover:underline"
                >
                  {hit.doc_id}
                </Link>
...
function ActiveChip({
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-zinc-900 text-white px-2 py-0.5">
      {label}
